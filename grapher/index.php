...
<div id=tour onclick="document.getElementById('tour').style.display='none';document.cookie='overlay=none';"
	...
	<table style='position:absolute;width:80%;height:80%;top:10%;left:10%;'>
		...
		<tr>
			...
			<td>
		<tr>
			<td><span>This left hand area displays your data (including the id on the left). You can add or delete rows as well as edit the data just by clicking and typing on it. To save the changes click the save changes in the menu.</span>
			<td colspan=2><span style='cursor: pointer;'
			                    onclick="document.getElementById('tour').style.display='none';document.cookie='overlay=none';">Okay, I got it.<br>Hide this overlay.</span>
		...
		<tr>
			...
			<td>
			<td>
			<td><span>Down here you can change the options for the graph, including the titles and if the point labels are shown.</span><br>
				<svg width="60" height="60">
					<defs>
						<marker id="arrow" markerWidth="13" markerHeight="13" refx="2" refy="6" orient="auto">
							<path d="M2,2 L2,11 L10,6 L2,2" style="fill:black;"/>
...
<div id=pastetext
	...
	<div style='position:absolute;top:50%;left:50%;width:400px;height:300px;z-index:12;text-align:center;padding:5px;margin-left:-205px;margin-top:-155px;'>
		<div style='position:absolute;padding-top:2px;padding-bottom:2px;left:0px;top:0px;width:100%; text-align:center;font-weight:bold;border:none;background-color:rgba(0,100,200,0.85);color:#fff;'
		     id=sampletitle>Paste your table here from Excel, Word or Google Sheets
		</div>
		<br>
		<small><i>Note: Pasting a table from Google Docs doesn't work properly.</i></small>
		<br>
...
<div id=pastelink
	...
	<div style='position:absolute;top:50%;left:50%;width:400px;height:300px;z-index:12;text-align:center;padding:5px;margin-left:-205px;margin-top:-155px;'>
		<div style='position:absolute;padding-top:2px;padding-bottom:2px;left:0px;top:0px;width:100%; text-align:center;font-weight:bold;border:none;background-color:rgba(0,100,200,0.85);color:#fff;'
		     id=sampletitle>Paste your link here
		</div>
		<br>
		<small><i>Note: the link needs to be accessible on the web for this to work, and must be to a CSV file.</i>
		</small>
		<br>
...
<div id=top>
	...
	<div style="position:absolute;top:18px;right:35px;background:none;border:none;">
		...
		<form id=datasource method=get style='display:inline;'>
			...
			Data Source: <select name=dataset onChange="document.getElementById('datasource').submit();"
	...
	<img src='3dots.png' id='3dots'
	     style="position:absolute;height:25px;right:15px;top:17px;z-index:10;cursor:pointer;">
</div>
...
<div id=helppopup class="callout popup">
	<ul>
		...
		<li onclick="window.open('//www.mathsnz.com/nzgrapher-info/dataset-information','_blank');">Dataset
			Information
		</li>
		...
		<li id=uploadfileclick>
			<form method="post" name=fileform id=uploadfile enctype="multipart/form-data">
				<input type="file" name="file" id="filebox"
				       ...
				       onChange="document.getElementById('uploadfile').submit();">
				<div style='color:#000;border:0px solid #ccc;width:80px;height:17px;border-radius:0px;padding:5px;font-size:14px;text-align:left;position:absolute;top:4px;left:6px;padding:0px;background:none;'>
					Open File
				</div>
			</form>
		</li>
...
<div id=left>
	<div id="samvardiv"
	     style="display:none;z-index:19;position:absolute;top:30px;left:30px;right:30px;bottom:30px;text-align:center;padding-top:30px;">
		<div style='position:absolute;padding-top:2px;padding-bottom:2px;left:0px;top:0px;width:100%; text-align:center;font-weight:bold;border:none;background-color:rgba(0,100,200,0.85);color:#fff;'
		     id=sampletitle>Sampling Variability
		</div>
		...
		<a href='#'
		   ...
		   id=samvargo>(Re)Sample</a><br><br><br>
			...
			<tr>
				...
				<td>
					<select style='width:120px' id=type onChange="graphchange(this);">
						...
						<option disabled></option>
						<option value='newpairedexperimentdotplot'>Paired Experiment Dot Plot (and Arrows Graph)
						</option>
						<option disabled></option>
		...
		<table style='width:100%;margin-bottom:5px;max-width: 800px;margin: 0 auto;'>
			<tr>
				<td style='width:50%;border-right:1px solid #ccc;padding-right:10px;vertical-align:top;padding-bottom:0px;padding-top:0px;'>
					...
					If you're not sure where to start watching <a target='_blank'
					                                              href='//www.mathsnz.com/nzgrapher-info/video-tutorials'>these
						videos</a> is a good idea.<br>
					<br>
					There is all the infomation on the graphs and datasets and tutorials on how to use NZGrapher over on
...
<div id=controls>
	<table style='width:100%;'>
		<tr>
			...
			<span id=arrowsshow>
				...
				<input type="checkbox" onclick="updategraph();" id="boxplot" name="boxplot"
				       value="yes"> Box Plots</label><br>
			</span>
				...
				<input type="checkbox" onclick="updategraph();" id="labels" name="labels"
				       value="yes"> Point Labels</label><br>
			</span>
					<span id=meandotshow><label>
				<input type="checkbox" onclick="updategraph();" id="meandot" name="meandot"
				       value="yes"> Mean Dot</label><br>
			</span>
				...
				<input type="checkbox" onclick="updategraph();" id="jitter" name="jitter"
				       value="yes"> Jitter</label><br>
			</span>
				...
				<input type="checkbox" onclick="updategraph();" id="exp" name="exp"
				       value="yes"> y=a*exp(b*x)</label><br>
			</span>
				...
				<input type="checkbox"
				       onclick="if(this.checked==true){$('#seasonal')[0].checked=true;}else{$('#seasonal')[0].checked=false;};updategraph();"
				       id="longtermtrend" name="longtermtrend" value="yes"> Long Term Trend (STL)</label><br>
			</span>
				...
				<input type="checkbox" onclick="updategraph();" id="invert" name="invert"
				       value="yes"> Invert Colours</label><br>
			</span>
				...
				<input type="checkbox" onclick="updategraph();" id="relativefrequency" name="relativefrequency"
				       value="yes"> Relative Freq.</label><br>
			</span>
					<span id=removedpointsshow><label>
				<input type="checkbox" onclick="updategraph();" id="removedpoints" name="removedpoints" value="yes"
				       checked> Show ID of Removed Points</label><br>
			</span>
			...
			<span style='display:inline-block;width:50px;'>x-axis: </span><input type="text" id="xaxis" name="xaxis"
			                                                                     value="X Axis Title"><br>
			<span style='display:inline-block;width:50px;'>y-axis: </span><input type="text" id="yaxis" name="yaxis"
			                                                                     value="Y Axis Title"><br>
			<span id=colorname><span style='display:inline-block;width:50px;'>Colour: </span><input type="text"
			                                                                                        id="colorlabel"
			                                                                                        name="colorlabel"
			...
			<td style="display:none" id="customSize">
                <span style='font-size: 12px; line-height:27px;'>
                    <span style="display:inline-block;width:50px;">Width: </span><input type="number" id="customWidth"
                                                                                        name="customWidth" value="1920"><br>
                    <span style="display:inline-block;width:50px;">Height: </span><input type="number" id="customHeight"
                                                                                         name="customHeight"
                                                                                         value="1080"><br>
                    <span style="display:inline-block;width:50px;">Scale: </span><input type="number"
                                                                                        id="customScalefactor"
                                                                                        name="customScalefactor"
                                                                                        value="1"><br>
                </span>
			</td>
		</tr>
	...
	<div id=transdiv>Transparency: <input id=trans type="range" min=0 max=100 step=10 value=50 onchange="updategraph()">
	</div>
	<div id=updater>
		<?php
		if (isset($_GET['dev'])) {
...
<div id="samplediv"
	...
	<div style='position:absolute;padding-top:2px;padding-bottom:2px;left:0px;top:0px;width:100%; text-align:center;font-weight:bold;border:none;background-color:rgba(0,100,200,0.85);color:#fff;'
	     id=sampletitle>Sample
	</div>
	<div style='position:absolute;right:7px;top:1px;background:none;border:none;cursor:pointer;color:#fff;' class=close>
		&times;
	</div>
	<br>
...
<div id="newvardiv"
     style="z-index:6;display:none;padding:10px;position:absolute;border:none;box-shadow: 0px 0px 10px rgba(0,0,0,0.5);top:50%;left:50%;-webkit-transform: translate(-50%,-50%);-ms-transform: translate(-50%,-50%);transform: translate(-50%,-50%);">
	<div style='position:absolute;padding-top:2px;padding-bottom:2px;left:0px;top:0px;width:100%; text-align:center;font-weight:bold;border:none;background-color:rgba(0,100,200,0.85);color:#fff;'
	     id=ordertitle>Create New Variable (From 2 Variables)
	</div>
	<div style='position:absolute;right:7px;top:1px;background:none;border:none;cursor:pointer;color:#fff;' class=close>
		&times;
	</div>
...
<div id="newvarcdiv"
     style="z-index:6;display:none;padding:10px;position:absolute;border:none;box-shadow: 0px 0px 10px rgba(0,0,0,0.5);top:50%;left:50%;-webkit-transform: translate(-50%,-50%);-ms-transform: translate(-50%,-50%);transform: translate(-50%,-50%);">
	<div style='position:absolute;padding-top:2px;padding-bottom:2px;left:0px;top:0px;width:100%; text-align:center;font-weight:bold;border:none;background-color:rgba(0,100,200,0.85);color:#fff;'
	     id=ordertitle>Create New Variable (Linear Function)
	</div>
	<div style='position:absolute;right:7px;top:1px;background:none;border:none;cursor:pointer;color:#fff;' class=close>
		&times;
	</div>
...
<div id="newvarc3div"
	...
	<div style='position:absolute;padding-top:2px;padding-bottom:2px;left:0px;top:0px;width:100%; text-align:center;font-weight:bold;border:none;background-color:rgba(0,100,200,0.85);color:#fff;'
	     id=ordertitle>Create New Variable (From Condition)
	</div>
	<div style='position:absolute;right:7px;top:1px;background:none;border:none;cursor:pointer;color:#fff;' class=close>
		&times;
	</div>
...
<div id="sortdiv"
     style="z-index:6;display:none;padding:10px;position:absolute;border:none;box-shadow: 0px 0px 10px rgba(0,0,0,0.5);top:50%;left:50%;-webkit-transform: translate(-50%,-50%);-ms-transform: translate(-50%,-50%);transform: translate(-50%,-50%);">
	<div style='position:absolute;padding-top:2px;padding-bottom:2px;left:0px;top:0px;width:100%; text-align:center;font-weight:bold;border:none;background-color:rgba(0,100,200,0.85);color:#fff;'
	     id=ordertitle>Sort Data by Variable
	</div>
	<div style='position:absolute;right:7px;top:1px;background:none;border:none;cursor:pointer;color:#fff;' class=close>
		&times;
	</div>
...
<div id="converttimediv"
     style="z-index:6;display:none;padding:10px;position:absolute;border:none;box-shadow: 0px 0px 10px rgba(0,0,0,0.5);top:50%;left:50%;-webkit-transform: translate(-50%,-50%);-ms-transform: translate(-50%,-50%);transform: translate(-50%,-50%);">
	<div style='position:absolute;padding-top:2px;padding-bottom:2px;left:0px;top:0px;width:100%; text-align:center;font-weight:bold;border:none;background-color:rgba(0,100,200,0.85);color:#fff;'
	     id=filtertitle>Convert Time Column
	</div>
	<div style='position:absolute;right:7px;top:1px;background:none;border:none;cursor:pointer;color:#fff;' class=close>
		&times;
	</div>
...
<div id="encodetimediv"
     style="z-index:6;display:none;padding:10px;position:absolute;border:none;box-shadow: 0px 0px 10px rgba(0,0,0,0.5);top:50%;left:50%;-webkit-transform: translate(-50%,-50%);-ms-transform: translate(-50%,-50%);transform: translate(-50%,-50%);">
	<div style='position:absolute;padding-top:2px;padding-bottom:2px;left:0px;top:0px;width:100%; text-align:center;font-weight:bold;border:none;background-color:rgba(0,100,200,0.85);color:#fff;'
	     id=filtertitle>Encode Time Column
	</div>
	<div style='position:absolute;right:7px;top:1px;background:none;border:none;cursor:pointer;color:#fff;' class=close>
		&times;
	</div>
...
<div id=cover
     style="width:100%;height:100%;position:fixed;top:0px;left:0px;background-color:rgba(255,255,255,0.5);z-index:20;display:none;"
     onclick="$('#cover').hide();$('#options').hide();"></div>
<div id=options
     style="width:35%;height:50%;position:fixed;top:20%;left:2.5%;border:1px solid #ccc;z-index:21;display:none;">
	<div style="background-color:rgba(0, 100, 200, 0.85096);width:100%;text-align:center;height:25px;position:absolute;top:0px;left:0px;border:none;color:#fff;">
		More Options
		<div style='position:absolute;right:7px;top:1px;background:none;border:none;cursor:pointer;color:#fff;'
		     onclick="$('#cover').hide();$('#options').hide();">&times;
		</div>
	</div>
	<div style='position:absolute;left:0px;top:25px;right:0px;bottom:0px;overflow:auto;padding:5px;font-size:12px;'>
		...
		<span href="#"
		      style="text-decoration:none;color:#fff;background-color:rgba(0,100,200,0.85);padding:10px;font-size:12px;cursor:pointer;"
		      onclick="$('#options input').val('auto');updategraph();">Reset</span>
		<span href="#"
		      style="text-decoration:none;color:#fff;background-color:rgba(0,100,200,0.85);padding:10px;font-size:12px;cursor:pointer;"
...
<div id=tooltip
	...
	<div id=tooltiparrow
	     style='background:none;z-index:99999;position:absolute;top:50%;margin-top:-8px;left:-8px;width: 0;height: 0;border-top: 8px solid transparent; border-bottom: 8px solid transparent; border-right:8px solid #000;'></div>
